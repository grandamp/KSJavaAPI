 ...
 * 
 * $Id: CommandAPDU.java 3 2013-07-23 16:00:13Z grandamp@gmail.com $
 *
...
//    private static final int MAX_APDU_SIZE = 65544;
    ...
    private void parse() {
        ...
        if (l1 != 0) {
            if (this.apdu.length == 4 + 1 + l1) {
                // case 3s
                this.nc = l1;
                this.dataOffset = 5;
                return;
            } else if (this.apdu.length == 4 + 2 + l1) {
                // case 4s
                this.nc = l1;
                this.dataOffset = 5;
                int l2 = this.apdu[this.apdu.length - 1] & 0xff;
                this.ne = (l2 == 0) ? 256 : l2;
                return;
            } else {
                throw new IllegalArgumentException
                    ("Invalid APDU: length=" + this.apdu.length + ", b1=" + l1);
            }
        }
        ...
        int l2 = ((this.apdu[5] & 0xff) << 8) | (this.apdu[6] & 0xff);
        if (this.apdu.length == 7) {
            // case 2e
        ...
        if (l2 == 0) {
            throw new IllegalArgumentException("Invalid APDU: length="
                    + this.apdu.length + ", b1=" + l1 + ", b2||b3=" + l2);
        }
        if (this.apdu.length == 4 + 3 + l2) {
            // case 3e
            this.nc = l2;
            this.dataOffset = 7;
            return;
        } else if (this.apdu.length == 4 + 5 + l2) {
            // case 4e
            this.nc = l2;
            this.dataOffset = 7;
            int leOfs = this.apdu.length - 2;
            int l3 = ((this.apdu[leOfs] & 0xff) << 8) | (this.apdu[leOfs + 1] & 0xff);
    ...
    public CommandAPDU(int cla, int ins, int p1, int p2, byte[] data,
        ...
        if (dataLength == 0) {
            ...
            if (ne == 0) {
                ...
                if (dataLength <= 255) {
                    // case 3s
                    this.apdu = new byte[4 + 1 + dataLength];
                    setHeader(cla, ins, p1, p2);
                    ...
                    // case 3e
                    this.apdu = new byte[4 + 3 + dataLength];
                    setHeader(cla, ins, p1, p2);
                    this.apdu[4] = 0;
                ...
                if ((dataLength <= 255) && (ne <= 256)) {
                    // case 4s
                    this.apdu = new byte[4 + 2 + dataLength];
                    setHeader(cla, ins, p1, p2);
                    this.apdu[4] = (byte)dataLength;
                    this.dataOffset = 5;
                    System.arraycopy(data, dataOffset, this.apdu, 5, dataLength);
                    ...
                    // case 4e
                    this.apdu = new byte[4 + 5 + dataLength];
                    setHeader(cla, ins, p1, p2);
                    this.apdu[4] = 0;
                    this.apdu[5] = (byte)(dataLength >> 8);
                    this.apdu[6] = (byte)dataLength;
